...
**iOS · Android** &nbsp;·&nbsp; **2024.11 → 2026.06** &nbsp;·&nbsp; 화면 약 197개 &nbsp;·&nbsp; 개발 2명 + 디자이너 1명
...
호스트 쪽만 화면이 80개가 넘습니다. 숙소 등록은 유형 → 위치 → 객실 → 편의시설 → 정책 순의 다단계 마법사이고, 정산은 주차 단위로 수수료를 빼고 입금 예정일까지 보여줍니다.
...
<table>
<tr>
<td align="center" width="25%"><img src="assets/screens/onboarding.png" width="165" /><br><sub><b>인트로</b></sub></td>
...
<td align="center" width="25%"><img src="assets/screens/house_manage.png" width="165" /><br><sub><b>숙소 정보 관리</b></sub></td>
</tr>
<tr>
...
서버 구성도 대신, **손가락이 화면에 닿은 뒤 무슨 일이 벌어지는지**로 대신하겠습니다. 지도를 한 번 미는 동작입니다.
...
onCameraMoveEnd()
    │
    ├──▶ 보이는 숙소 개수 갱신 ──────────────▶ 즉시 (네트워크 안 탐)
    │
    ▼
300ms 기다린다 ─── 아직 움직이면 취소 ───▶ ✕
    │
    ▼
직전 경계와 거의 같은가? ── 예 ──────────▶ ✕
    │ 아니오
...
화면이 덮는 타일 번호 계산
    │
    ▼
캐시에 없는 타일만 추림 ── 없으면 ───────▶ ✕ (네트워크 안 탐)
    │ 있으면
    ▼
ApiClient.get()  ──── 401이면 ──▶ 공유 락으로 토큰 1회 갱신 후 재시도
    │
    ▼
응답을 타일별로 흩어 담고 캐시에 기록
    │
    ▼
화면 다각형 안에 드는 것만 골라 마커로 그린다
```

세 번의 `✕`가 이 구조의 핵심입니다. **가장 빠른 요청은 나가지 않는 요청**입니다.

<details>
<summary>앱 전체 구성 보기</summary>
<br>
...
main()
 ...
 ├─ 푸시 · 알림 초기화
 └─ MultiProvider (ChangeNotifier 30개)
      └─ 로그인 여부 분기
           ├─ 온보딩 / 로그인
           └─ 홈 셸
...
|---|---|---|---|
| **197** | **231** | **254** | **30** |
| 화면 | Dart 파일 | 위젯 클래스 | ChangeNotifier |
| **100** | **5** | **271** | **159** |
| REST 엔드포인트 | 로컬 DB 테이블 | 커밋 | Pull Request |

</div>

가장 큰 덩어리는 호스트 콘솔이고, 그다음이 프로필 · 여행 · 메시지 · 예약 달력 순입니다.
