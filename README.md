...
손가락을 뗀다
    ...
    ▼
① 300ms 기다린다 ── 아직 움직이는 중 ─────▶ 취소
    ▼
② 직전과 거의 같은 영역인가? ── 예 ────────▶ 종료
    ▼
③ 화면이 덮는 타일 중 캐시에 없는 것만 추림
    │
    ├── 없으면 ──────────────────────────▶ 종료 (네트워크 안 탐)
    ▼
   서버 요청 → 타일별로 흩어 담고 캐시에 기록
    ▼
   화면 다각형 안에 드는 것만 마커로 그린다
```
...
화면 197개가 서로를 어떻게 붙들고 있는지의 이야기입니다.
...
```
   ...
   ┌───────────────┬─────────────────┬─────────────────┬──────────────────┐
   ▼               ▼                 ▼                 ▼                  ▼
ApiClient     SocketManager      SQLite          포그라운드 태스크    Platform Channel
토큰 갱신 락    하트비트·재연결    채팅·프로필 캐시    업로드 큐 영속화     지도 SDK · 위치
```
...
**화면은 서로를 직접 부르지 않습니다.** 예약을 수락한 화면이 달력 화면을 직접 갱신하지 않고, 가운데 상태 계층에 "바뀌었다"고만 알립니다. 화면이 200개인 앱에서 화면끼리 직접 얽히기 시작하면 어디서 무엇이 바뀌는지 추적이 불가능해집니다. → [상태 관리](docs/03-state-management.md)
