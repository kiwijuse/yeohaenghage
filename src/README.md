...
> 실제 앱은 250개 Dart 파일로 이루어져 있고, 그중 설계 의도가 잘 드러나는 세 덩어리만 골라
...
- **디버그 `print()` 제거.** 원본 코드베이스 전체에 847곳이 있었습니다.
...
변수명은 원본의 `snake_case`를 그대로 두었습니다. Dart 권장 스타일(`lowerCamelCase`)과는 다르지만, 실제 코드베이스가 그렇게 되어 있고 여기서만 예쁘게 고치면 발췌본으로서 정직하지 않기 때문입니다.
