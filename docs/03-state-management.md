...
Provider가 해결한 건 "데이터를 어디에 둘 것인가"가 아니라 **"데이터가 바뀌었다는 사실을 누구에게 언제 알릴 것인가"**였습니다.
...
마지막 항목이 실제로 부담이 됐습니다. **최종적으로 ChangeNotifier가 30개**까지 늘었고, 전부 하나의 파일에 모여 있습니다. 새로 합류한 사람이 "이 값은 어디서 바뀌나"를 찾으려면 파일 전체를 읽어야 합니다.
...
이 작업에서 배운 건 라이브러리 사용법이 아니라 **공유의 단위를 정하는 감각**이었습니다.

값을 공유할지, 신호만 공유할지. 하나로 묶을지, 종류별로 쪼갤지. 전부 구독할지, 일부만 구독할지. 이 세 가지 판단이 성능과 유지보수를 결정했고, 라이브러리는 그 판단을 실행하는 도구였을 뿐입니다.
